        ...
        private Dictionary<string, string> GetListContent(XPathNavigator navigator, string xpath, string contentType, ITripleSlashCommentParserContext context)
        {
            ...
            foreach (XPathNavigator nav in iterator)
            {
                ...
                if (!string.IsNullOrEmpty(name))
                {
                    ...
                    {
                        string path = 
                            context.Source is null ? context.CodeSourceBasePath ?? "Unknown source" :
                            context.Source.Remote != null ? Path.Combine(EnvironmentContext.BaseDirectory, context.Source.Remote.RelativePath) : context.Source.Path;
